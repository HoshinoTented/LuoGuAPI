...
function renderData(data, status, score, time, memory) {
    ...
    if (status || status === 0) {
        ...
        if (status !== 1 && status !== 0) {
            $.get("/recordnew/ajaxGetData/14149207", function (data) {
    ...
    if (status === 12) {
        $.get("/record/ajax_rate?rid=14149207", function (data) {
            var res;
            res = eval("(" + data + ")");
            if (res["code"] === 200) {
                hide_artile();
...
$(document).ready(function () {
    ...
    renderData({
        ...
        "subtasks": [{"status": 14, "score": 0, "time": 24, "memory": 940, "judger": 1}]
    }, 14, 0, 24, 940);
    $("#record-region").show();
});